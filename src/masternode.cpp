...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight)
{
    if (nHeight <= 500) {
        ...
        return 100000 * COIN;
    } else if (nHeight <= 1575000) {
        return 150000 * COIN;
    } else if (nHeight <= 2100000) {
        return 200000 * COIN;
    } else if (nHeight <= 2625000) {
        return 250000 * COIN;
    } else if (nHeight <= 3150000) {
        return 250000 * COIN;
    } else if (nHeight <= 3675000) {
        return 250000 * COIN;
    } else if (nHeight <= 4200000) {
        return 250000 * COIN;
    } else if (nHeight <= 4725000) {
        return 250000 * COIN;
    } else if (nHeight > 4725000) {
        return 250000 * COIN;
    }
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 nMNSC tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 nMNSC tx -> 1 confirmation
