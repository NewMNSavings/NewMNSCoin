...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("nMNSC client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("nMNSC"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("nMNSC"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
