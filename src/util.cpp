...
const char * const PIVX_CONF_FILENAME = "nmnsc.conf";
const char * const PIVX_PID_FILENAME = "nmnsc.pid";
...
// Mac: ~/Library/Application Support/nmnsc
// Unix: ~/.nmnsc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "nMNSC";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nmnsc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nmnsc.conf
